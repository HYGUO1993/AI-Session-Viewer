...
pub fn global_search(
    ...
    let results: Vec<SearchResult> = match source {
        ...
        "codebuddy" => search_codebuddy(&query_lower, max_results, scope),
...
fn search_codebuddy(
    ...
    let mut sessions = Vec::new();
    for project in codebuddy::get_projects().unwrap_or_default() {
        let meta = metadata::load_metadata("codebuddy", &project.id);
        for session in codebuddy::get_sessions(&project.id).unwrap_or_default() {
            let session_meta = meta.sessions.get(&session.session_id);
            sessions.push((
                project.id.clone(),
                project.short_name.clone(),
    ...
    let results: Vec<SearchResult> = sessions
        ...
        .flat_map(|(project_id, project_name, session, alias, tags)| {
            ...
            let Ok(messages) = codebuddy::parse_all_messages(std::path::Path::new(&session.file_path))
            else {
                return Vec::new();
            };
            let mut search_aliases = Vec::with_capacity(2);
